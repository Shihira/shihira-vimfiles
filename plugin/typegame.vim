...
def retshf(l):shuffle(l);return l
curl = [chr(c) for c in range(ord(' ') + 1,ord('A')) + 
        range(ord('Z') + 1,ord('a')) + \
        range(ord('z') + 1,127) ]
cons = ["".join(retshf(curl)) for i in range(10)]
#vim.vars defaultly refers to global variables
vim.vars["__typegame_handler__"] = cons
eof

                let src_con = g:__typegame_handler__
        elseif(a:doc =~ "{.*}")
                echoerr "Built-in docment '" . a:doc . "' is not defined."
                return
        else
                let src_con = readfile(a:doc)
        endif
...
function! g:ToggleType(...)
        if (a:0 == 0) | let doc = "{CurBuf}"
        else | let doc = a:1 | endif

        let tbnr =  s:InitTypeBuffer(doc)
        normal 2G0
...
command! -nargs=? ToggleType call g:ToggleType(<f-args>)
