...
function! perforce#P4GetUser()
  if exists("t:perforce_user") && len(t:perforce_user) > 0
      return t:perforce_user
  endif

  let l:output = s:P4Shell('info')
  let l:m = matchlist(output, "User name: \\([a-zA-Z]\\+\\).*")
  if len(l:m) > 1 && !empty(l:m[1])
    let t:perforce_user = l:m[1]
    return t:perforce_user
  endif
endfunction

function! perforce#P4GetClient()
  if exists("t:perforce_client") && len(t:perforce_client) > 0
      return t:perforce_client
  endif

  let l:user = perforce#P4GetUser()
  let l:output = s:P4Shell('clients -u '.l:user)
  for l:line in l:output->split('\n')
      let l:m = l:line->matchlist("Client \\(\\S\\+\\) \\([0-9/]\\+\\) root \\(\\S\\+\\) ")
      let l:root = l:m[3]
      if l:root[len(l:root) - 1] != g:sep
          let l:root .= g:sep
      endif
      if expand('%:p')[0:len(l:root) - 1] == l:root
          let t:perforce_client = l:m[1]
          return t:perforce_client
      endif
  endfor
...
" @bind-menu: Perforce.Info
function! perforce#P4CallInfo()
  let $P4CLIENT = perforce#P4GetClient()
  let l:output = s:P4Shell('info')
  echo l:output
endfunction
...
" @bind-menu: Perforce.Edit
function! perforce#P4CallEdit()
  let $P4CLIENT = perforce#P4GetClient()
  
  if exists('t:perforce_changelist') && !empty(t:perforce_changelist)
    let l:output = s:P4ShellCurrentBuffer('edit -c '.t:perforce_changelist)
  else
...
function! perforce#P4GetUserPendingChangelists()
  let l:client = perforce#P4GetClient()
  if !empty(client)
    let l:output = s:P4Shell('changes -s pending -c ' . l:client)
    if v:shell_error
      return ''
    endif
    if exists('t:perforce_changelist') && !empty(t:perforce_changelist)
        return l:output->substitute("Change ".t:perforce_changelist." ", "[*] \\0", "g")
    else
        return l:output
    endif
  endif
...
" @bind-menu: Perforce.Move\ To\ Changelist
function! perforce#P4CallPromptMoveToChangelist()
  ...
  nnoremap <buffer> <silent> <CR>   : exe perforce#P4ConfirmMoveToChangelist(getline('.'), 0) <CR>
  nnoremap <buffer> <silent> <C-CR>   : exe perforce#P4ConfirmMoveToChangelist(getline('.'), 1) <CR>
  " Populate buffer with existing Changelists
  execute "normal! GiNew changelist\<cr>default\<cr>" . l:user_cls . "\<esc>ddgg2gg"
  silent! setlocal autoread nomodifiable
endfunction

function! perforce#P4ConfirmMoveToChangelist(changelist_str, set_current)
  ...
  if !empty(l:target_cl)
    if a:set_current
      let t:perforce_changelist = l:target_cl
    endif
